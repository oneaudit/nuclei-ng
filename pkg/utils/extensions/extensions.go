...
var (
	...
	SecondDenylist  = []string{".css", ".js", ".docx", ".pptx", ".scss", ".sass", ".ts", ".log", ".vcf", ".srt", ".sql", ".sqlite", ".cmd", ".bat", ".sh", ".ini", ".conf", ".bin", ".bin"}

	// AllowedList any check must use the lowercase extension
	AllowedList = []string{".html", ".php", ".php5", ".asp", ".aspx", ".jsp", ".xml", ".twig"}
...
func IsHTMLFile(extension string) bool {
	...
	for _, nonHtmlExtension := range SecondDenylist {
		if strings.EqualFold(extension, nonHtmlExtension) {
			return false
		}
	}
